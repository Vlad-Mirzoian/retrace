...
describe("createProgram — replay", () => {
  it("opens the replay view for the given session and registers a SIGINT handler to stop it", async () => {
    const stop = vi.fn().mockResolvedValue(undefined);
    const handle: UiHandle = { url: "http://localhost:1234", port: 1234, stop };
    const startReplay = vi.fn().mockResolvedValue(handle);

    const program = createProgram({ createStore: () => store, startReplay });
    await program.parseAsync(["node", "retrace", "replay", "sess-1"]);

    expect(startReplay).toHaveBeenCalledTimes(1);
    const [passedStore, sessionId, options] = startReplay.mock.calls[0];
    expect(passedStore).toBe(store);
    expect(sessionId).toBe("sess-1");
    expect(options.port).toBeUndefined();
  ...
  it("parses --port and passes it through as a number", async () => {
    ...
    const program = createProgram({ createStore: () => store, startReplay });
    await program.parseAsync(["node", "retrace", "replay", "sess-1", "--port", "4321"]);

    const [, , options] = startReplay.mock.calls[0];
    expect(options.port).toBe(4321);
    process.emit("SIGINT");
  });
  ...
  it("passes openBrowser: false when --no-open is given", async () => {
    ...
    const program = createProgram({ createStore: () => store, startReplay });
    await program.parseAsync(["node", "retrace", "replay", "sess-1", "--no-open"]);
